library scissors.src.css_pruning.signature_generator;

import 'package:csslib/visitor.dart';

class SignatureGenerator extends Visitor {
  final TreeOutput output;
  RuleSet _currRuleSet;
  final bool useSpan;
  List<RuleSet> lr = new List();
  List<Declaration> ld = new List();
  Map<RuleSet, List<Declaration>> mrd = new Map<RuleSet,List<Declaration>>();
  SignatureGenerator(this.output, this.useSpan) {
    output.printer = this;
  }
  ...
  void visitDeclaration(Declaration node) {
    ...
    output.write(_currRuleSet.span.text);
    lr.add(_currRuleSet);
    ld.add(node);
    if(mrd[_currRuleSet] == null)
      mrd[_currRuleSet] = [];
    mrd[_currRuleSet].add(node);
